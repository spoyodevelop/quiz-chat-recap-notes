...
import { BookOpen, MessageCircle, CheckSquare, Sparkles } from 'lucide-react';
...
const Index = () => {
  ...
  const [learningPhase, setLearningPhase] = useState<'input' | 'chat' | 'board'>('input');
              ...
              <CardContent className="space-y-6">
                ...
                {completedSessions.length > 0 && (
                  <div className="pt-6 border-t">
                    <h3 className="text-sm font-semibold text-gray-600 mb-3">최근 학습 주제</h3>
                    <div className="flex flex-wrap gap-2">
                      {completedSessions.slice(-5).reverse().map((session) => (
                        <Badge key={session.id} variant="secondary" className="bg-blue-100 text-blue-700">
                          {session.topic}
                        </Badge>
                      ))}
                    </div>
